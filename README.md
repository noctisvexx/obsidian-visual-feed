...
一个 Obsidian 插件：扫描你指定的文件夹（每日随笔、社交平台 归档、Memos、剪藏……），把里面出现过的**图片和视频**抽出来，重新排成 Instagram 风格的时间流 —— 单列大图，或者多列网格瀑布流，左下角一点就切。
...
用久了 Obsidian 之后，照片是**散落**的：今天的记录在 `notes/journal/2026/0901.md`，一条嘟文在 `notes/social/2026-09-01.md`，随手记在另一个地方。想回看「我以前拍过什么」，只能在文件树里翻。
...
| **分组方式** | 每条记录一个 Post（社交平台 每条嘟文、Memos 每条随手记各自成帖）／每个文件一个 Post |
